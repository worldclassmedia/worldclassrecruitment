...
<?php require ("scripts/admin_includes.php"); ?>
...
<title>Add Job | World Class Recruitment</title>

<?php //Success Messages
if ($_GET["step"] === "2" & $_GET["status"] == NULL) :?>
	<div id="success_message">Now add some screening questions or skip the screening questions and Add Job.</div>
<?php endif;

if ($_GET["status"] == "added_question") :?>
	<div id="success_message">You've successfully added a screening question. You can either add some more or finnish by clicking Add Job</div>
<?php endif;?>

<?php if ($_GET["status"] == "deleted_question") :?>
	<div id="success_message">You've successfully deleted a screening question. You can either add some more or finnish by clicking Add Job</div>
<?php endif;?>
...
<div id='addjob_header_holder' class='clearfix'>
   ...
   <a href="dashboard.php">
        <img id='return_icon' src='img/return_icon.png' class='image' />
   </a>
</div>
<div id='add_job_form' class='clearfix'>
   <div id='job_form_holder' class='clearfix'>
        <form
        <?php if($_GET["step"] !== "2"): ?>
        action="scripts/add_job_step1.php" 
        <?php else: ?>
        action="scripts/add_job_step2.php?id=<?php echo $_GET["id"];?>"
        <?php endif;?>
        name="add_job_form" method="post" onSubmit="return form_checker()">
                    
<?php if($_GET["id"] !== NULL):
//Run while to spit out currently working on job ?>
...
<?php else: //Step 1?>			
           ...
           <div id='job_title_textarea' class='clearfix'>
               <input id="form_job_title" name="job_title" type="text" style="resize:none">
           </div>
       </div>
       <div id='salary_holder' class='clearfix'>
           <p id='salary'>
           ...
           <div id='salary_textarea' class='clearfix'>
            <input id="form_salary" name="salary" type="text"  style="resize:none">
           </div>
       </div>
       <div id='description_holder' class='clearfix'>
           <p id='description'>
           ...
           <div id='description_textarea' class='clearfix'>
             <textarea id="form_description" name="description" type="text" style="resize:none"></textarea>
           </div>
       </div>
       <div id='person_spec_holder' class='clearfix'>
           <p id='person_spec'>
           ...
           <div id='person_spec_textarea' class='clearfix'>
                <textarea id="form_person_spec" name="person_spec" type="text" style="resize:none"></textarea>
           </div>
           <div id="select_catergory_list" class="clearfix dropdown dropdown-dark">
               <select name="category_id" class="dropdown-select">
...
<?php endif;?>
   ...
   <?php endif; ?>
   <input type="submit" id="admin_submit" class="submit_button" name="add_job"
   <?php if($_GET['step'] == "2"): ?>
        value="Add Job" 
   <?php else: ?>
        value="Next"
   ...
   >
   <?php if($_GET['step'] == "2"): ?>
        <input type="submit" id="admin_submit" class="add_question_button" name="add_question" onClick="return check_question()" value="Add Question"> 
   <?php endif; ?>
